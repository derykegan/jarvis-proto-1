...
/**
 * Simple speech output based on the FreeTTS demo.
 */
public class SpeechOutput {
	
	static String voiceToUse = "kevin16";			// Using default voice
	static boolean engineSetup = false;			// Flag to indicate engine ready
	
	Synthesizer synthesizer;
	
	/**
	 * Default constructor - use kevin16
	 */
	public SpeechOutput(){
		doStartup();
	}
	...
	/**
	 * Alt constructor - use custom voice
	 */
	public SpeechOutput(String voice){
		voiceToUse = voice;
		doStartup();
	}
	
	/**
	 * Actual startup actions.
	 */
	private void doStartup(){
		try {
            ...
            for (int i = 0; i < voices.length; i++) {
                if (voices[i].getName().equals(voiceToUse)) {
                    voice = voices[i];
                    break;
                }
            }
	        ...
	        // Indicate system is ready for output
	        setEngineReadyFlag(true);
	        
		    /* The the synthesizer to speak and wait for it to
	             * complete.
	             */
		    synthesizer.speakPlainText("JARVIS online.", null);
	...
	/**
	 * Converts the provided string to voice.
	 * 
	 * @param toSpeak The string to be converted to speech.
	 * @throws AudioException
	 * @throws EngineStateError
	 * @throws IllegalArgumentException
	 * @throws InterruptedException
	 */
	public void speechOutput(String toSpeak) throws AudioException, EngineStateError, IllegalArgumentException, InterruptedException{
		// if the engine is not ready, do nothing
		if(!ready()){
			return;
		}
		...
		//otherwise we will send the string for output
	...
	/**
	 * Set the status of the engine - once initialised should be true
	 * @param flag
	 */
	private void setEngineReadyFlag(boolean flag){
		engineSetup = flag;
	}
	
	/**
	 * Set the status of the engine - once initialised should be true
	 * @return boolean for engine state
	 */
	...
	/**
	 * Cleanly shut down speech output.
	 */
	public void close(){
		try {
